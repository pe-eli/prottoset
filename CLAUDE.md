...
| IA — WhatsApp | DeepSeek Chat → geração de mensagens WhatsApp |
...
- `backend/src/modules/whatsapp/whatsapp.queue.ts` — fila WhatsApp com geração DeepSeek + SSE
...
| `/leads/whatsapp` | WhatsAppBlastPage (disparo via Evolution API + DeepSeek AI) |
...
| POST | `/api/whatsapp/blast` | Inicia fila WhatsApp (DeepSeek + Evolution), retorna `blastId` |
...
- **Fase 1 — Geração**: para cada lote, chama `deepseekService.generateWhatsAppMessage(promptBase)`. Emite SSE `generating` e `batch_message` com a mensagem gerada
