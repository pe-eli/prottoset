...
export function LoginPage({ onAuthenticated }: LoginPageProps) {
  ...
  const [mobileBlocked, setMobileBlocked] = useState<boolean>(() => {
  ...
  const busy = loading || googleLoading || resendingVerification;
  ...
  useEffect(() => {
    ...
    const handleChange = (event: MediaQueryListEvent) => {
      setMobileBlocked(event.matches);
    };

    setMobileBlocked(mediaQuery.matches);
      ...
      >
        <div className="w-full max-w-md animate-slide-up">
          <div className="bg-surface-elevated rounded-2xl border border-border-light p-6 shadow-2xl shadow-black/20 text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-400">Acesso restrito</p>
            ...
            <p className="mt-4 text-sm text-text-secondary">
              Use um desktop para continuar.
            </p>
            <p className="mt-2 text-xs text-text-muted">
              O login e o painel foram desativados para dispositivos móveis.
            </p>
            <Link
              to="/"
              className="mt-5 inline-flex items-center justify-center font-semibold rounded-xl transition-all duration-200 px-5 py-2.5 text-sm border border-brand-400/40 text-brand-300 hover:bg-brand-400/10"
            >
      ...
      <div className="w-full max-w-md animate-slide-up">
        <div className="bg-surface-elevated rounded-2xl border border-border-light p-6 shadow-2xl shadow-black/20">
          ...
          <button
            ...
            disabled={busy}
            className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-xl border border-border text-sm font-medium text-text-primary hover:bg-surface-secondary transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {googleLoading ? (
              <>
                <span className="h-4 w-4 rounded-full border-2 border-current border-t-transparent animate-spin" />
          ...
          <p className="mt-5 text-xs text-text-muted text-center">
            {mode === 'login' ? (
              <>
                Não tem conta?{' '}
                <button type="button" onClick={switchMode} disabled={busy} className="text-brand-400 hover:underline font-medium disabled:opacity-60 disabled:cursor-not-allowed">
                  Criar conta
                </button>
              </>
            ) : (
